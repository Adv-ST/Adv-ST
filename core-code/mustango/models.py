...
class MusicAudioDiffusion(nn.Module):
	...
	# @torch.no_grad()
	def inference2(self, latents, prompt, beats, chords,chords_time, inference_scheduler, num_steps=20, guidance_scale=3, num_samples_per_prompt=1, 
				  disable_progress=True):
		device = self.text_encoder.device
		classifier_free_guidance = guidance_scale > 1.0
		batch_size = len(prompt) * num_samples_per_prompt
	...
	def prepare_latents(self, batch_size, inference_scheduler, num_channels_latents, dtype, device):
		# shape = (batch_size, num_channels_latents, 256, 16)
		shape = (batch_size, num_channels_latents, 64, 16)
		latents = randn_tensor(shape, generator=None, device=device, dtype=dtype)
		# scale the initial noise by the standard deviation required by the scheduler
		latents = latents * inference_scheduler.init_noise_sigma
		return latents
