...
class AMSoftmaxLoss(torch.nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        assert logits.size(0) == labels.size(0), "Logits and labels should have the same batch size"
...
class Attack:
    def __init__(self, tgtm, cycle_flag, audio, impulse, phrase, freq_min, freq_max, batch_size, learning_rate, weight_decay, outdir, bp, psy):
        self.bp = bp
        self.psy = psy
        self.outdir = outdir
        self.phrase = phrase
        ...
        # logging.info(self.tango.model.PE.global_time_embedding.angles.requires_grad)
        # logging.info(self.tango.model.FME.translation_bias.requires_grad)
        # pdb.set_trace()
        ...
        self.get_logits = get_logits(tgtm, cycle_flag, self.outdir, torch.randn(1,Fs*2), self.phrase)
    ...
    def run_step(self, itr):
        ...
        # for p in self.tango.model.text_encoder.get_input_embeddings().parameters():
        #     logging.info(p.grad)
        self.optimizer.step()

        decoded = [str(te) for te in texts]
        return decoded, logits, ce_loss, final_input, final_input, final_input, return_texts_token
    ...
    def attack(self, outdir, num_iterations=5000):
        ...
        for itr in range(num_iterations + 1):
            ...
            # set(['i love', 'you make me sick']) == set(['you make me sick', 'i love'])
...
def main():
    ...
    For now we only support generating one adversarial music at a time.
    """
    parser = argparse.ArgumentParser(description=None)
    parser.add_argument('--in', type=str, dest='input', required=False,
                        help='Input audio .wav file, at {fs}Hz'.format(fs=Fs))
    parser.add_argument('--imp', type=str, dest='impulse', nargs='+', required=False,
    ...
    attack = Attack(args.tgtm, args.cycle, None, None, args.target, freq_min=args.freq_min, freq_max=args.freq_max,
