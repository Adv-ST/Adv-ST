...
class get_logits():
    ...
    def logits(self, wav, teacher_forcing):
        ...
        if teacher_forcing:
            ...
            for now_index in range(len(use_target_lang)):
                ...
                for i in range(prompt_len, target_decoder_output_token_sequence.shape[-1]):
                    ...
                    ctc_loss = ctc_loss + ce_loss_func(log_probs.squeeze(1), target_decoder_output_token_sequence[:, i])
        ...
        logging.info("\n" + "*" * 40)
        logging.info(texts)
        return None, texts, ctc_loss, return_texts_token
...
class Attack:
    def __init__(self, audio, impulse, phrase, freq_min, freq_max, batch_size, learning_rate, weight_decay, outdir, bp, psy):
        self.bp = bp
        self.psy = psy
        self.outdir = outdir
        self.phrase = phrase
    ...
    def run_step(self, itr):
        ...
        # input
        final_input = wave[0]
        logits, texts, ctc_loss, return_texts_token = self.get_logits.logits(final_input, True)
        if (itr) % 10 == 0:
        ...
        sum_loss = ctc_loss + psy_loss
        # sum_loss = quality_loss*100. + ctc_loss
        # sum_loss = quality_loss*100. + psy_loss
        ...
        return decoded, logits, ctc_loss, final_input, final_input, final_input, return_texts_token
    ...
    def attack(self, outdir, num_iterations=5000):
        ...
        for itr in range(num_iterations + 1):
            ...
            decoded, logits, ctcloss, ae_transformed, ae_input, delta_filtered, return_texts_token = self.run_step(itr)
...
def main():
    ...
    attack = Attack(None, None, args.target, freq_min=args.freq_min, freq_max=args.freq_max,
    ...
    pattern = re.compile(r"(.+)-adv-(\d+)\.wav$")
    for filename in os.listdir(args.out):
        ...
        if match:
            ...
            if max_b is None or b > max_b:
                ...
                max_b_file = filename
    if max_b:
        if max_b >= args.iterations or max_b%1000 != 0:
            logging.info(("#"*30+"\n")*3)
            logging.info(f"already end with {max_b_file}")
            return 0
