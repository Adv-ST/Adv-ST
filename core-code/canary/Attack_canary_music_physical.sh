...
if [[ ! -z "$audio_file" ]]; then
    ...
    for lang_combo in "${language_combinations[@]}"; do
        for index in "${!sentences[@]}"; do
            ...
            out_folder="Generated/Attack-canary-(${lang_suffix})-music-physical/${audio_file_name}/${sentence_index}"
            mkdir -p "$out_folder"
            command="python Attack_canary_music_physical.py --in \"$audio_file\" \
                    ...
                    --src_lang "eng" \
                    --iterations 5 \
                    --speech_pth /public/liuchang/data/LibriSpeech_wav/ "
